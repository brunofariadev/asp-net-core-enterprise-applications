...
namespace NSE.Bff.Compras.Controller
{
    [Authorize]
    public class CarrinhoController : MainController
    {
        ...
        public async Task<IActionResult> RemoverItemCarrinho(Guid produtoId)
        {
            ...
            if (produto == null)
            {
                AdicionarErro("Produto inexistente!");
        ...
        public async Task<IActionResult> AplicarVoucher([FromBody] string voucherCodigo)
        {
            ...
            if (voucher is null)
            {
                AdicionarErro("Voucher inválido ou não encontrado!");
                return CustomResponse();
            }
        ...
        {
            if (produto == null) AdicionarErro("Produto inexistente!");
            if (quantidade < 1) AdicionarErro($"Escolha ao menos uma unidade do produto {produto.Nome}");
            ...
            {
                AdicionarErro($"O produto {produto.Nome} possui {produto.QuantidadeEstoque} unidades em estoque, você selecionou {quantidade}");
                return;
            }

            if (quantidade > produto.QuantidadeEstoque) AdicionarErro($"O produto {produto.Nome} possui {produto.QuantidadeEstoque} unidades em estoque, você selecionou {quantidade}");
        }
    }
}
